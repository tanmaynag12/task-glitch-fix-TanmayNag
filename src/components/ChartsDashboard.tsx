...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  // Exclude invalid ROI values to prevent NaN in charts
  const validRoiTasks = tasks.filter(
    (t) => typeof t.roi === "number" && Number.isFinite(t.roi)
  );
  const roiBuckets = [
    { label: "<200", count: validRoiTasks.filter((t) => t.roi! < 200).length },
    {
      label: "200-500",
      count: validRoiTasks.filter((t) => t.roi! >= 200 && t.roi! <= 500).length,
    },
    { label: ">500", count: validRoiTasks.filter((t) => t.roi! > 500).length },
    { label: "N/A", count: tasks.length - validRoiTasks.length },
  ];
